...
class Profile(models.Model):
    ...
    APPROVAL = (
        ('TO_BE_APPROVED', 'Chờ phê duyệt'),
        ('APPROVED', 'Đã phê duyệt'),
        ('NOT_APPROVED', 'Không được phê duyệt'),
    )
    HAIR_LENGTH = (
        ('LONG', 'Dài'),
        ('SHOULDER_LENGTH', 'Dài tới vai'),
        ('AVERAGE', 'Trung bình'),
        ('SHORT', 'Ngắn'),
        ('SHAVED', 'Cạo trọc'),
    )
    ...
    EDUCATION = (
        ...
        ('COLLEGE', 'Cao đẳng'),
        ('BACHELORS_DEGREE', 'Cử nhân'),
        ('MASTERS', 'Thạc sĩ'),
        ('PHD_POST_DOCTORAL', 'Tiến sĩ / Sau tiến sĩ'),
    )
    GENDER = (
        ("MALE", "Nam"),
        ("FEMALE", "Nữ"),
    ...
    gender = models.CharField(
        choices=GENDER, default="MALE", max_length=6, blank=False)
    hair_length = models.CharField(
        choices=HAIR_LENGTH, default="LONG", max_length=20, blank=False)
    # ethnicity = models.CharField(
    #     choices=ETHNICITY, default="WHITE", max_length=20, blank=False)
    relationship_status = models.CharField(
        choices=RELATIONSHIP_STATUS, default="NEVER_MARRIED", max_length=20, blank=False)
    education = models.CharField(
        choices=EDUCATION, default="HIGH_SCHOOL", max_length=20, blank=False)
    ...
    hair_colour = models.CharField(
        choices=HAIR_COLOUR, default="BLACK", max_length=10, blank=False)
    body_type = models.CharField(
        choices=BODY_TYPE, default="AVERAGE", max_length=15, blank=False)
    looking_for = models.CharField(
        choices=LOOKING_FOR, default='BOTH', max_length=6, blank=False)
    children = models.BooleanField(default=False)
    location = models.CharField(max_length=100, blank=False)
    citylat = models.DecimalField(
        max_digits=9, decimal_places=6, default=-2.0180319)  # Sửa default thành số
    citylong = models.DecimalField(
        max_digits=9, decimal_places=6, default=52.5525525)  # Sửa default thành số
    birth_date = models.DateField(
        null=True, blank=True)  # Xóa default để tránh lỗi
    is_premium = models.BooleanField(default=False)
    is_verified = models.CharField(
        choices=APPROVAL, default="TO_BE_APPROVED", max_length=14, blank=False)
...
class ProfileImage(models.Model):
    ...
    image = models.ImageField(upload_to=image_filename,
                              blank=True, default='avatars/default_avt.png')
    is_verified = models.CharField(
        choices=APPROVAL, default="TO_BE_APPROVED", max_length=14, blank=False)
