...
class UserRegistrationForm(UserCreationForm):
    ...
    def cleaned_email(self):
        ...
        if User.objects.filter(email=email).exclude(username=username):
            raise forms.ValidationError(u'Email đã được sử dụng. Vui lòng thử lại.')
    ...
    def cleaned_password2(self):
        ...
        if not password1 or password2:
            raise ValidationError("Mật khẩu không được để trống. Vui lòng thử lại.")
...
class EditProfileForm(UserChangeForm):
    ...
    def clean_confirm_password(self):
        ...
        if not self.user.check_password(password):
            raise forms.ValidationError("Nhập sai mật khẩu. Vui lòng thử lại.")
        return password
