...
def delete(request):
    try:
        ...
        messages.success(
            request, "Có lỗi xảy ra. Vui lòng liên hệ chúng tôi để biết thêm chi tiết")
    return redirect(reverse('preregister'))
...
def create_profile(request):
    ...
    ImageFormSet = modelformset_factory(
        ProfileImage, form=ProfileImageForm, extra=6, max_num=6, help_texts=None)

    if request.method == "POST":
        profile_form = ProfileForm(request.POST, instance=request.user.profile)
        image_form = ProfileImageForm(request.POST, request.FILES)
        formset = ImageFormSet(request.POST, request.FILES, queryset=ProfileImage.objects.filter(
        ...
        initial_images = [{'image_url': i.image} for i in ProfileImage.objects.filter(
            user_id=request.user.id).all() if i.image]
        formset = ImageFormSet(queryset=ProfileImage.objects.filter(
            user_id=request.user.id).all(), initial=initial_images)
...
def member_profile(request, id):
    ...
    try:
        ...
        # Debug
        print(
            f"Images for {member.username}: {[(img.image.url, img.is_verified) for img in images]}")
    ...
    if not member == request.user:
        ...
        result = looking_for_check(
            request, request.user.profile, member.profile)
        if result:
            return result
        result = looking_for_check(
            request, member.profile, request.user.profile)
        if result:
            return result

        # Thêm lượt xem nếu chưa xem hoặc lượt xem trước chưa được đọc
        last_view = Views.objects.filter(receiver_id=id).filter(
            sender_id=request.user.id).last()
            ...
            if message_form.is_valid():
                ...
                if request.user.profile.is_premium:
                    ...
                    conversation = Conversations.objects.filter(
                        participants=request.user.id).filter(participants=id)
                    ...
                    messages.error(
                        request, "Bạn cần gói premium để gửi tin nhắn")
